...
export const TicketScannerModal: React.FC<TicketScannerModalProps> = ({
  ...
  // Manual input & image preview state
  const [manualCode, setManualCode] = useState('');
  const [uploadedImagePreview, setUploadedImagePreview] = useState<string | null>(null);
  const [copiedId, setCopiedId] = useState(false);

  // Stop camera and scanner safely
  const stopCamera = useCallback(() => {
    try {
      ...
      console.warn('Error while stopping ZXing scanner:', err);
    }
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Device enumeration notice:', devErr);
      }

      // Prioritize back/environment camera if deviceId not specified
      ...
      // Start continuous decode from video stream using @zxing/library
  ...
  // Image Upload handler with ZXing + jsQR fallback
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        if (result && result.getText()) {
          handleValidatePayload(result.getText());
          return;
        }
      } catch (zxingErr) {
            ...
            {!isVerifying && !validationResult && activeTab === 'camera' && (
              <div className="space-y-3">
                <div className="relative aspect-[4/3] w-full max-w-md mx-auto bg-black rounded-2xl overflow-hidden border-2 border-emerald-500/50 shadow-inner flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center p-8">
                    <div className="relative w-48 h-48 sm:w-56 sm:h-56 border-2 border-dashed border-emerald-400/80 rounded-2xl flex items-center justify-center">
                      ...
                      <div className="absolute left-2 right-2 h-0.5 bg-gradient-to-r from-transparent via-emerald-400 to-transparent shadow-[0_0_14px_rgba(52,211,153,0.95)] animate-pulse" />
                    </div>
                  </div>

                  {/* Top Bar inside Viewfinder */}
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[11px] font-mono bg-black/70 backdrop-blur-md px-3 py-1.5 rounded-xl border border-emerald-500/40">
                    ...
                    <div className="flex items-center gap-1.5">
                      {hasTorch && (
                        <button
                          ...
                          className="flex items-center gap-1 text-stone-300 hover:text-white bg-white/10 px-2 py-0.5 rounded-lg cursor-pointer"
                        >
                          {torchOn ? <Zap className="w-3 h-3 text-amber-300" /> : <ZapOff className="w-3 h-3 text-stone-400" />}
                          <span>Torch</span>
                        </button>
                      )}
                    ...
                    <div className="absolute inset-0 bg-stone-950/95 p-6 flex flex-col items-center justify-center text-center space-y-3 z-10">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-stone-800 hover:bg-stone-700 text-white text-xs font-bold transition-colors cursor-pointer"
                        >
                          Enter ID Code
                        </button>
                      </div>
                ...
                <p className="text-center text-xs text-stone-400">
                  Align attendee pass QR code within the frame to automatically verify and check in.
                </p>
              </div>
            )}
                ...
                <label className="border-2 border-dashed border-emerald-500/40 hover:border-emerald-400/80 rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center gap-3 cursor-pointer bg-black/30 hover:bg-emerald-950/20 transition-all text-center">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-stone-400">
                      Decodes via @zxing/library — Supports PNG, JPG, or screenshot of digital pass
                    </span>
                  </div>
                  <input
                    type="file"
                ...
                {uploadedImagePreview && (
                  <div className="p-3 bg-black/40 rounded-xl border border-stone-800 flex items-center gap-3">
                    ...
                    <div className="text-xs">
                      ...
                      <span className="text-stone-400">Processing barcode via ZXing...</span>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className="pt-2 border-t border-stone-800 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-stone-800/80 hover:bg-stone-700 border border-stone-700 text-stone-300 text-xs font-medium flex items-center gap-1.5 cursor-pointer"
                    >
                      <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                      <span>Generate & Validate Sample Pass</span>
                    </button>
                  </div>
                </div>
              </div>
